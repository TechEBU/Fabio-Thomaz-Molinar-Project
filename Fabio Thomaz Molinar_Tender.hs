...
-- Need to defined an instance of Eq only because I really need an instance of Ord
...
transition :: Tender -> State TenderDatum -> TenderRedeemer -> Maybe (TxConstraints Void Void, State TenderDatum)
transition tender s r = case (stateValue s, stateData s, r) of
    -- Creating the state machine
    (v, TenderDatum Nothing Nothing, Create)            -> Just ( Constraints.mustBeSignedBy (tenderOwner tender)
                                                                , State (TenderDatum Nothing Nothing) (lovelaceValueOf $ txCost tender))
    -- BIDING
    -- First bid
    (v, TenderDatum Nothing Nothing, Bid bs')           -> Just ( Constraints.mustPayToPubKey (tenderOwner tender) token
                                                                , State (TenderDatum (Just [bs']) Nothing) (lovelaceValueOf $ txCost tender))
    -- Next bids.
    (v, TenderDatum (Just bs) Nothing, Bid bs')         -> Just ( Constraints.mustPayToPubKey (tenderOwner tender) token
                                                                , State (TenderDatum (fmap ((++) [bs']) bs) Nothing) (lovelaceValueOf $ txCost tender))
    -- REVEALING
    -- First reveal
    (v, TenderDatum (Just bs) Nothing, Reveal bs' bi')
        | maybeToBool $ fmap (bs' `elem`) bs            -> Just ( Constraints.mustPayToPubKey (tenderOwner tender) token <>
                                                                  Constraints.mustValidateIn (from $ 1 + minBidTime tender)
                                                                , State (TenderDatum (Just bs) [bi']) (lovelaceValueOf $ txCost tender))
    -- Next reveals
    (v, TenderDatum (Just bs) (Just bi), Reveal bs' bi')
        | maybeToBool $ fmap (bs' `elem`) bs            -> Just ( Constraints.mustPayToPubKey (tenderOwner tender) token <>
                                                                  Constraints.mustValidateIn (from $ 1 + minBidTime tender)
                                                                , State (TenderDatum (Just bs) (fmap ((++) [bi']) bi)) (lovelaceValueOf $ txCost tender))
    -- Finish
    (v, TenderDatum (Just bs) (Just bi), Finish)        -> Just ( Constraints.mustBeSignedBy (tenderOwner tender) <>
                                                                  Constraints.mustValidateIn (from $ 1 + minRevealTime tender)
                                                                , State (TenderDatum (Just bs) (Just bi)) (lovelaceValueOf $ txCost tender))
    -- Cancel
    (v, d, Cancel)                                      -> Just ( Constraints.mustBeSignedBy (tenderOwner tender)
                                                                , State d (lovelaceValueOf $ txCost tender))
    -- Default
    _                                                   -> Nothing
  where
    token :: Value
...
bidInfoToBS bi = sha2_256 (appendByteString (getPubKeyHash $ bidOwner bi) (consByteString $ bidValue bi))
...
tenderStateMachine :: Tender -> ByteString -> StateMachine TenderDatum TenderRedeemer
...
mkTenderValidator :: Tender -> ByteString -> TenderDatum -> TenderRedeemer -> ScriptContext -> Bool
...
tenderClient tender = mkStateMachineClient $ StateMachineInstance (tenderStateMachine' tender) (tenderInst tender)
...
createTender :: forall w s. HasBlockchainActions s => CreateParams -> Contract w s Text ()
createTender cp = do
    ...
    case m of
        ...
        Just ((o, _), _) -> case tyTxOutData o of
            TenderDatum _ _ -> do
                ...
                void $ mapError' $ runStep client $ Bid $ bidInfoToBS $ bpBidInfo bp
...
revealTender :: forall w s. HasBlockchainActions s => RevealParams -> Contract w s Text ()
revealTender rp = do
    pkh <- pubKeyHash <$> Contract.ownPubKey
    let tender = Tender
    ...
    case m of
        ...
        Just ((o, _), _) -> case tyTxOutData o of
            TenderDatum _ _ -> do
                logInfo @String "Running tender found."
                void $ mapError' $ runStep client $ Reveal $ (bidInfoToBS $ rpBidInfo bp) (rpBidInfo rp)
...
cancelTender :: forall w s. HasBlockchainActions s => CancelParams -> Contract w s Text ()
cancelTender cp = do
    pkh <- pubKeyHash <$> Contract.ownPubKey
    let tender = Tender
            { tenderOwner     = pkh
...
endpoints = (create `select` bid `select` reveal `select` cancel `select` finish) >> endpoints
  where
